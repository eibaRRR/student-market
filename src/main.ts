...
const Router = {
  ...
  updateActiveNav() {
    // Left empty or implement sidebar active states if needed
  },
  ...
  updateNavbar() {
    ...
    if (user) {
      container.innerHTML = `
        <button onclick="window.router.navigate('student')" class="w-full h-12 flex items-center rounded-2xl bg-white/5 hover:bg-white/10 text-white transition-all overflow-hidden shrink-0">
          <div class="w-12 h-12 flex items-center justify-center shrink-0">
             <img src="${user.avatar}" class="w-8 h-8 rounded-full border border-white/20 shrink-0" alt="Avatar">
          </div>
          <span class="ml-2 font-bold whitespace-nowrap opacity-0 transition-opacity">Profil</span>
        </button>
        ${user.role === 'admin' ? `
        <button onclick="window.router.navigate('admin')" class="w-full h-12 flex items-center rounded-2xl bg-[#8000FF]/20 hover:bg-[#8000FF]/40 text-[#8000FF] transition-all overflow-hidden shrink-0">
          <div class="w-12 h-12 flex items-center justify-center shrink-0">
        ...
        <button onclick="window.router.components.logoutUser()" class="w-full h-12 flex items-center rounded-2xl hover:bg-red-500/20 text-slate-400 hover:text-red-500 transition-all overflow-hidden shrink-0">
          <div class="w-12 h-12 flex items-center justify-center shrink-0">
      ...
      container.innerHTML = `
        <button onclick="window.router.navigate('auth')" class="w-full h-12 flex items-center rounded-2xl hover:bg-white/10 text-white transition-all overflow-hidden shrink-0 border border-white/20">
          <div class="w-12 h-12 flex items-center justify-center shrink-0">
            <svg class="w-6 h-6 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"></path></svg>
          </div>
...
// Remove Theme Toggle Setup since we force dark mode
function setupTheme() {
...
function setupNav() {
  const overlaySearch = document.getElementById('overlay-search-input') as HTMLInputElement;
